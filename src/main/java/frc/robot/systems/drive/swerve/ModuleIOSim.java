...
public class ModuleIOSim implements ModuleIO {
  ...
  private PIDController driveController = new PIDController(1.0, 0.0, 0.0, LOOP_PERIOD_SECS);
  private SimpleMotorFeedforward driveFF = new SimpleMotorFeedforward(0, 0, 0);
  private PIDController turnController = new PIDController(1.0, 0.0, 0.0, LOOP_PERIOD_SECS);
  private SimpleMotorFeedforward turnFF = new SimpleMotorFeedforward(0, 0, 0);
  ...
  public void updateInputs(ModuleIOInputs inputs) {
    ...
    inputs.drivePositionMeters = DriveConstants.kWheelRadius * driveSim.getAngularPositionRad();
    inputs.driveVelocityMetersPerSec =
        DriveConstants.kWheelRadius * driveSim.getAngularVelocityRadPerSec();
  ...
  public void setPosition(double degrees) {
    setTurnVoltage(
        turnController.calculate(Math.toRadians(turnSim.getAngularPositionRad()), degrees)
            + turnFF.calculate( Math.signum( turnController.getPositionError() ) ) );
  }
}
